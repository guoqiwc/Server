...
class CSUserBehaviorRequestMessage
{
	 ...
	 * 
	 */
	public $_menuList = array ();

	/**
	 * 本次使用屏幕取色的次数
	 * 
	 */
	private $_takeColorTimes;
	...
	public function __construct($bin) 
	{
		settype ( $this->_takeColorTimes, 'integer' );
		settype ( $this->_macId, 'string' );
		...
		for($i = 0; $i < $count; ++ $i)
		{
			...
			$pb->setType ( $bt->readShort() );
			$pb->setTimeStamp ( $bt->readLongString() );
			array_push ( $this->_menuList, $pb);
		}
		$this->_takeColorTimes = $bt->readShort();
		$this->_macId = $bt->readLongString();
	...
	/**
	 * 本次使用屏幕取色的次数
	 * @return the $_takeColorTimes
	 * 
	 */
	public function getTakeColorTimes()
	{
		return $this->_takeColorTimes;
	}
